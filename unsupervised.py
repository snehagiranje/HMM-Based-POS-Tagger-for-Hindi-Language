...
def get_feature_vectors(tokens, unique_words, feature_words):
	...
	for each in xrange(len(unique_words)):
		feature_vectors.append([])
		for x in xrange(2* len(feature_words)):
...
def dist(data_point, centriod):
	...
	for x in xrange(len(data_point)):
...
def mean_of_data_points(feature_vectors, list_of_points):
	...
	mean = []
	for x in xrange(len(feature_vectors[0])):
		mean.append(0)
...
def main():
	...
	for x in xrange(len(cluster_data_map)):
	...
	for x in xrange(len(file_contents)):
		...
		for word in line:
			for i in xrange(len(cluster_data_map)):
